...
impl Calculator {
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        if let Some(repeat_this_op) = self.history.get(operation_index).cloned() {
            let result = repeat_this_op
            .operation_type
            .perform(repeat_this_op.first_num, repeat_this_op.second_num)?;
            self.history.push(repeat_this_op);
            Some(result)
        } else {
            None
        }
